...
server <- shinyServer(function(input, output, session) {
  ...
  {observeEvent(input$submit_survey, once = FALSE, {
    ...
    state <- TRUE # hard code to true to save time when testing
    if(state == TRUE){
      ...
      updateActionButton(session = session,
                         inputId = "submit_survey",
                         label = "Please answer all questions.")
    }
  })
...
ui <- shinyUI(
  navbarPage(title = "MOOC App", id = "mooc_app", collapsible = TRUE, selected = "survey",
    theme = shinythemes::shinytheme("sandstone"),
        ...
        fluidPage(
          fluidRow(
            column(8, offset = 2,
              ...
              h4("Please answer all of the questions below and click the \"Sumbit responses\" button at the 
                 bottom of the page.", align = "center"),
              hr(),
              ),
            ),
          fluidRow(
      ...
      tabPanel(title = "Radar charts", value = "radar_charts",
        {fluidPage(
          fluidRow(
